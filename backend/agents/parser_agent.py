...
2. Work Experience: For each position held:
   ...
   - Extract technologies and tools mentioned in that role into skills_used.
...
5. Certifications: List any professional licenses, certificates, or courses mentioned.
6. Summary: Include the professional bio or objective if present.
7. Preserve exact wording where possible for verifiable grounding. Do not hallucinate qualifications."""
...
class ParserAgent:
    ...
    def parse_and_anonymize(
        ...
        if isinstance(source, bytes) or (isinstance(source, (str, Path)) and str(source).lower().endswith(".pdf")):
